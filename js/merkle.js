...
function tlvTypeBytes(tlv) {
    ...
    const first = tlv[0];
    const width = first < 0xfd ? 1 : first === 0xfd ? 3 : first === 0xfe ? 5 : 9;
    if (tlv.length < width) {
...
function branchNode(left, right) {
    const ordered = compareBytes(left, right) <= 0 ? left.concat(right) : right.concat(left);
...
function merkleLevels(tlvs) {
    ...
    const firstTlv = tlvs[0];
    const levels = [tlvs.map(tlv => leafNode(tlv, firstTlv))];
...
function merkleRoot(tlvs) {
    const levels = merkleLevels(tlvs);
...
function resolveNode(node, nextHash) {
    ...
    const left = resolveNode(node.left, nextHash);
    const right = resolveNode(node.right, nextHash);
...
function reconstructRoot(leaves, missingHashes) {
    ...
    const root = resolveNode(buildTree(leaves), nextHash);
    if (root === OMITTED_SUBTREE) {
        throw new Error('Malformed request: every tlv in the tree is omitted');
    }
    if (at !== missingHashes.length) {
        throw new Error(`Malformed request: ${missingHashes.length - at} unused proof_missing_hashes`);
    }
    return root;
}
